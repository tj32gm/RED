...
local rd = 0.001
local by = 1989
local ylist = "2015 2045 2075"
local mlist = "01 02 03 04 05 06 07 08 09 10 11 12"

cd /media/tack/8GBlack/CMIP6_fromERA5/original/`md'_`vv'/regridded_new/
...
foreach y in `ylist' {
 ...
 insheet using tasmax_day_`md'_`sp'_`y'_`m'.txt, clear
 ...
 *ren coarse_center_lat group_center_lat
 *ren coarse_center_lon group_center_lon
 ...
 *tw scatter coarse_center_lat coarse_center_lon if month == 1 & day == 1 & year == 2015, msize(tiny)

 save tasmax_day_`md'_`sp'_`y'_`m'_regridded_new.dta, replace
}
}
...
foreach y in `ylist' {
 ...
 use tasmax_day_`md'_`dt'_`y'_`m'_regridded_new.dta, clear
 ...
 ** number of outliers **
 ...
 save `vv'_day_`md'_`dt'_`y'_`m'_regridded_new_clean.dta, replace
}
}
}
...
** keeping only historical for ssp126 & 245 **
...
foreach m in `mlist' {
 di " ------------  month `m' -------------  "
 use    `vv'_day_`md'_`dt'_1979_`m'_regridded_new_clean.dta, clear
...
*************** Append all years (SSP 245 + Historical) *****************
* hist data are generated in the analysis of ssp585
...
foreach m in `mlist' {
 ...
 use                  `vv'_day_`md'_historical_1979_`m'_regridded_new_clean.dta, clear
 append using `vv'_day_`md'_ssp585_2015_`m'_regridded_new_clean.dta
 append using `vv'_day_`md'_ssp585_2045_`m'_regridded_new_clean.dta
 append using `vv'_day_`md'_ssp585_2075_`m'_regridded_new_clean.dta
 
 
 * match lat and lon from hist and ssp
 gen lat_r = round(lat,`rd')
 replace lat = lat_r
 gen lon_r = round(lon,`rd')
 replace lon = lon_r
 drop lat_r lon_r
 
 sort lat lon month day year
 bysort lat lon month day (year): gen nn = _N

 * check if nn == 121 (# of years)

 di " ----- nn = 121? (2099-1979+1 = 121)----- "
 tab nn
 drop nn
 ...
 * remove duplicates
 ...
 save          `vv'_day_`md'_all_`m'_regridded_new.dta, replace
}
...
foreach m in `mlist' {
 ...
 use  `vv'_day_`md'_all_`m'_regridded_new.dta, clear
 
 keep if year >= 1979 & year <= 2099
 
 keep if coarse_grid_id == `r'
 ...
 * creating base year *
 ...
 save /media/tack/8GBlack/CMIP6_fromERA5/original/`md'_`vv'_seasonXregion/final_`vv'_`md'_regridded_month`m'_region`r'_base`by'.dta, replace
...
******* create 10 year intervaled data and record  **********
...
gen `vv'_r2 = round(`vv',0.2)
gen `vv'_r3 = round(`vv',0.4)
gen `vv'_r4 = round(`vv',0.5)

replace `vv' = `vv'_r1 if year <= 2030
replace `vv' = `vv'_r2 if year > 2030 & year <= 2050
replace `vv' = `vv'_r3 if year > 2050 & year <= 2070
replace `vv' = `vv'_r4 if year > 2070
...
bysort loc month shuffle day (year): gen nn = _N
}
* check if nn == 11 (# of years in each part)
di " ----- nn = 11? ----- "
tab nn
...
forvalues p = 1/11 {
  ...
  * create allmonth data
  forvalues r = 1/`nloc' {
    use                  final_`vv'_`md'_regridded_month01_region`r'_base`by'_round`rd'_record_part`p'.dta, clear
    append using final_`vv'_`md'_regridded_month02_region`r'_base`by'_round`rd'_record_part`p'.dta
   ...
   use                  final_`vv'_`md'_regridded_month03_region`r'_base`by'_round`rd'_record_part`p'.dta, clear
   append using final_`vv'_`md'_regridded_month04_region`r'_base`by'_round`rd'_record_part`p'.dta
   append using final_`vv'_`md'_regridded_month05_region`r'_base`by'_round`rd'_record_part`p'.dta
   save                final_`vv'_`md'_regridded_season2_region`r'_base`by'_round`rd'_record_part`p'.dta, replace

   use                  final_`vv'_`md'_regridded_month06_region`r'_base`by'_round`rd'_record_part`p'.dta, clear
   append using final_`vv'_`md'_regridded_month07_region`r'_base`by'_round`rd'_record_part`p'.dta
   append using final_`vv'_`md'_regridded_month08_region`r'_base`by'_round`rd'_record_part`p'.dta
   save                final_`vv'_`md'_regridded_season3_region`r'_base`by'_round`rd'_record_part`p'.dta, replace
   
   use                  final_`vv'_`md'_regridded_month09_region`r'_base`by'_round`rd'_record_part`p'.dta, clear
...
clear
gen year = .
save   /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`sp'_`md'_RED_regridded_allmonthXallregion_base`by'_round`rd'.dta, replace
...
** check if # of grid is 69 for each month & year **
*egen my = group(year month)
*tabstat coarse_grid_id, s(max) by(my)
...
append using /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`sp'_`md'_RED_regridded_allmonthXallregion_base`by'_round`rd'.dta
save                /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`sp'_`md'_RED_regridded_allmonthXallregion_base`by'_round`rd'.dta, replace
...
*************** 2. compute RED : regional (allmonth x region) ******************
...
clear
gen year = .
save  /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_allmonthXregion_base`by'_round`rd'.dta, replace
...
append using  /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_allmonthXregion_base`by'_round`rd'.dta
save                 /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_allmonthXregion_base`by'_round`rd'.dta, replace
...
cd   /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/
...
clear
gen year = .
save /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_seasonXallregion_base`by'_round`rd'.dta, replace
...
append using /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_seasonXallregion_base`by'_round`rd'.dta
save                /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_seasonXallregion_base`by'_round`rd'.dta, replace
...
clear
gen year = .
save  /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_seasonXregion_base`by'_round`rd'.dta, replace
...
append using /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_seasonXregion_base`by'_round`rd'.dta
save                /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion/final_`md'_RED_regridded_seasonXregion_base`by'_round`rd'.dta, replace
...
save final_`sp'_`md'_RED_regridded_seasonXregion_base`by'_round`rd'_clean.dta, replace
...
* create allmonth data
forvalues r = 1/`nloc' {
    use                  final_`vv'_`md'_regridded_month01_region`r'_base`by'_round`rd'_record_single.dta, clear
    append using final_`vv'_`md'_regridded_month02_region`r'_base`by'_round`rd'_record_single.dta
   ...
   use                  final_`vv'_`md'_regridded_month03_region`r'_base`by'_round`rd'_record_single.dta, clear
   append using final_`vv'_`md'_regridded_month04_region`r'_base`by'_round`rd'_record_single.dta
   append using final_`vv'_`md'_regridded_month05_region`r'_base`by'_round`rd'_record_single.dta
   save                final_`vv'_`md'_regridded_season2_region`r'_base`by'_round`rd'_record_single.dta, replace

   use                  final_`vv'_`md'_regridded_month06_region`r'_base`by'_round`rd'_record_single.dta, clear
   append using final_`vv'_`md'_regridded_month07_region`r'_base`by'_round`rd'_record_single.dta
   append using final_`vv'_`md'_regridded_month08_region`r'_base`by'_round`rd'_record_single.dta
   save                final_`vv'_`md'_regridded_season3_region`r'_base`by'_round`rd'_record_single.dta, replace
   
   use                  final_`vv'_`md'_regridded_month09_region`r'_base`by'_round`rd'_record_single.dta, clear
...
** computing REDH based on single data: global (allmonth X allregion) **
...
* number of years: 2099-1989+1=111 *
local ny  = 111
...
save /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`md'_RED_regridded_allmonthXallregion_base`by'_round`rd'_single.dta, replace
...
*************** 4. single data; compute RED : season  x  region ******************
...
clear
gen year = .
save  /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`md'_RED_regridded_seasonXregion_base`by'_round`rd'_single.dta, replace

forvalues ss = 1/4 {
 forvalues rr = 1/`nloc' {

use final_`vv'_`md'_regridded_season`ss'_region`rr'_base`by'_round`rd'_record_single.dta, clear
...
append using /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`md'_RED_regridded_seasonXregion_base`by'_round`rd'_single.dta
save                /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`md'_RED_regridded_seasonXregion_base`by'_round`rd'_single.dta, replace
