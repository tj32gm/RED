...
local md = "MPI-ESM1-2-LR"
local dt = "ssp585"
local vv = "tmax"

cd F:/CMIP6_sub/MPI-ESM1-2-LR

insheet  using  flexible_coarse_grid_mapping_merged.txt, clear
...
save flexible_coarse_grid_mapping_merged.dta, replace
...
insheet using  tasmax_day_`md'_`dt'_`y'_`m'.txt, clear
...
merge n:1 lat lon using flexible_coarse_grid_mapping_merged.dta
...
save tasmax_day_`md'_`dt'_`y'_`m'_regridded.dta, replace
...
qui {
use 		         tasmax_day_`md'_historical_1979_`m'_regridded.dta, clear
append using  tasmax_day_`md'_ssp585_2015_`m'_regridded.dta
append using  tasmax_day_`md'_ssp585_2058_`m'_regridded.dta
...
qui {
save  tasmax_day_`md'_all_month`m'_regridded.dta, replace

forvalues r = 1/`nloc' {
 use   tasmax_day_`md'_all_month`m'_regridded.dta, clear
...
* create the 10-year intervaled data *
...
gen `vv'_r2 = round(`vv',0.05)
gen `vv'_r3 = round(`vv',0.1)

replace `vv' = `vv'_r1 if year <= 2030
replace `vv' = `vv'_r2 if year > 2030 & year <= 2050
replace `vv' = `vv'_r3 if year > 2050
...
tabstat recordmax, s(mean) by(year)
