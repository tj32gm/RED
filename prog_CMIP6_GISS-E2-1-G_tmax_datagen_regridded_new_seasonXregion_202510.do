...
local md = "GISS-E2-1-G"
local dt = "ssp585"
local vv = "tmax"

cd F:/CMIP6_sub/GISS-E2-1-G
...
*************  import data *****************
* large file is broken into 10 equal size in time. So each split file is not clear cut by month or year.

local md = "GISS-E2-1-G"
local vv = "tmax"

local dt = "ssp585"
local ylist = "2015 2018 2022 2025 2029 2033 2036 2040 2043 2047 2051 2055 2060 2065 2070 2075 2080 2085 2090 2095"

local dt = "historical"
local ylist = "1979 1981 1983 1985 1987 1990 1992 1994 1996 1998 2000 2001 2003 2004 2006 2007 2009 2010 2012 2013"
...
foreach y in `ylist' {
 ...
 insheet using  tasmax_day_`md'_`dt'_split_regridded_`y'.txt, clear
 ...
 *tw scatter coarse_center_lat coarse_center_lon if month == 1 & day == 1 & year == 2015, msize(tiny)
 ...
 save tasmax_day_`md'_`dt'_`y'_regridded_new.dta, replace
}
...
local dt = "historical"
local ylist1 = "1981 1983 1985 1987 1990 1992 1994 1996 1998 2000 2001 2003 2004 2006 2007 2009 2010 2012 2013"

use tasmax_day_`md'_`dt'_1979_regridded_new.dta, clear

foreach y in `ylist1' { 
 append using tasmax_day_`md'_`dt'_`y'_regridded_new.dta
}
sort lat lon year month day
bysort lat lon year month day: keep if _n == 1

save tasmax_day_`md'_`dt'_all_regridded_new.dta, replace

local dt = "ssp585"
local ylist1 = "2018 2022 2025 2029 2033 2036 2040 2043 2047 2051 2055 2060 2065 2070 2075 2080 2085 2090 2095"

use tasmax_day_`md'_`dt'_2015_regridded_new.dta, clear
foreach y in `ylist1' { 
 append using tasmax_day_`md'_`dt'_`y'_regridded_new.dta
}
sort lat lon year month day
...
** create monthly and regional data : ssp585 **
...
forvalues m = 1/12 {
 forvalues r = 1/`nloc' {
  di "  --------------  month `m'  & region == `r' ----------------- "
  use   tasmax_day_`md'_ssp585_all_regridded_new.dta, clear
  keep if month == `m' & coarse_grid_id == `r'
  save tasmax_day_`md'_ssp585_month`m'_region`r'_regridded_new.dta, replace
}
}


** create monthly and regional data : historical **

forvalues m = 1/12 {
 forvalues r = 1/`nloc' {
  di "  --------------  month `m'  & region == `r' ----------------- "
  use   tasmax_day_`md'_historical_all_regridded_new.dta, clear
  keep if month == `m' & coarse_grid_id == `r'
  save tasmax_day_`md'_historical_month`m'_region`r'_regridded_new.dta, replace
}
}

** append historical and ssp585 **
local md = "GISS-E2-1-G"
local vv = "tmax"
local nloc  = 69
local rd = 0.001
...
forvalues m = 1/12 {
 forvalues r = 1/`nloc' {
  di "  --------------  month `m'  & region == `r' ----------------- "
  use                  tasmax_day_`md'_historical_month`m'_region`r'_regridded_new.dta, clear
  append using tasmax_day_`md'_ssp585_month`m'_region`r'_regridded_new.dta
  save                tasmax_day_`md'_all_month`m'_region`r'_regridded_new.dta, replace
 }
}
...
qui {
use tasmax_day_`md'_all_month`m'_region`r'_regridded_new.dta, clear
 ...
 ** number of outliers **
 ...
 save tasmax_day_`md'_all_month`m'_region`r'_regridded_new_clean.dta, replace
}
}
...
forvalues m = 1/12 {
 ...
 use  tasmax_day_`md'_all_month`m'_region`r'_regridded_new_clean.dta, clear
 ...
 * creating base year *
 ...
 save /media/tack/8GBlack/CMIP6_fromERA5/original/`md'_`vv'_seasonXregion/tasmax_day_`md'_all_month`m'_region`r'_regridded_clean_base`by'.dta, replace

}
}


** add record for 10 year intervaled data **
...
qui {
use tasmax_day_`md'_all_month`m'_region`r'_regridded_clean_base`by'.dta, clear
