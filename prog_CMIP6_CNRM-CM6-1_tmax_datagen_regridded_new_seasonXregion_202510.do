...
cd /media/tack/8GBlack/CMIP6_fromERA5/original/`md'_tmax

insheet  using  /media/tack/Climate-7/CPC_daily/stata_files/flexible_coarse_grid_mapping_merged.txt, clear
...
forvalues y = 2015(30)2075 {
 forvalues m = 1(1)12 { 
 
di " -------------- year `y' & month `m' ------------------- "

insheet using  tasmax_`md'_`dt'_y`y'_m`m'.txt, clear
...
qui {
 forvalues r = 1/`nloc' {
  use  tasmax_day_`md'_`dt'_`y'_`m'_regridded.dta, clear
  keep if month == `m' & coarse_grid_id ==`r'
  save tasmax_day_`md'_`dt'_`y'_`m'_regridded_month`m'_region`r'.dta, replace
  }
}
}
}
...
use                   tasmax_day_`md'_historical_1979_`m'_regridded_month`m'_region`r'.dta, clear
append using  tasmax_day_`md'_ssp585_2015_`m'_regridded_month`m'_region`r'.dta
append using  tasmax_day_`md'_ssp585_2045_`m'_regridded_month`m'_region`r'.dta
append using  tasmax_day_`md'_ssp585_2075_`m'_regridded_month`m'_region`r'.dta
...
gen `vv'_r3 = round(`vv',0.3)
gen `vv'_r4 = round(`vv',0.4)
...
append using /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`sp'_`md'_RED_regridded_allmonthXregion_base`by'_round`rd'.dta
save                 /media/tack/8GBlack/CMIP6_fromERA5/final_seasonXregion_`sp'/final_`sp'_`md'_RED_regridded_allmonthXregion_base`by'_round`rd'.dta, replace
...
use final_`sp'_`md'_RED_regridded_allmonthXregion_base`by'_round`rd'.dta, clear
...
save final_`sp'_`md'_RED_regridded_allmonthXregion_base`by'_round`rd'.dta, replace
...
save final_`md'_RED_regridded_seasonXregion_base`by'_round`rd'.dta, replace
